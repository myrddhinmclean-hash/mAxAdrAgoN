---
module: "module-0.5"
session: "0.5"
title: "FairWhether Market — Stat Blocks"
act: "I"
assumed_party_level: 1
system: "D&D 5e (SRD 5.1 / 2014)"
status: "Ratification-pending (numeric slate)"
rulings_applied:
  - "Ruling 42: D&D 5e SRD 5.1 rules baseline"
  - "Ruling 42a: Attrition-first tone guard"
---

# FairWhether Market — Combat & Threat Statistics

All stat blocks reference spatial areas and line numbers in `campaign/golden-sea/modules/module-0.5.md`.

---

## Area 1: Market Square (Module Lines 85–94)
* **Encounter Type:** Social tension / Public confrontation.
* **Evasion / Parley Viability:** High. The 8 kobold conscripts seek to enforce a market curfew and will avoid combat if challenged by the crowd or redirected by a distraction.
* **Ruling 42a Tone Guard:** Conscripts rout immediately on the first casualty. A frontal assault risks drawing the entire garrison.

### Kobold Conscript (8 Present)
*Small humanoid (kobold), lawful evil*
* **Module Source:** `module-0.5.md` L64 ("One of the eight kobolds in the writ detail... The kobold is a conscript"), L166 (`The Kobold Conscript with the Red Armband`).
* **Armor Class:** 12 (leather armor)
* **Hit Points:** 5 (2d6 - 2)
* **Speed:** 30 ft.
* **STR:** 7 (-2) | **DEX:** 15 (+2) | **CON:** 9 (-1) | **INT:** 8 (-1) | **WIS:** 7 (-2) | **CHA:** 8 (-1)
* **Senses:** Darkvision 60 ft., Passive Perception 8
* **Traits:**
  * *Pack Tactics:* Advantage on attack rolls against a creature if at least one ally is within 5 ft. of the target and not incapacitated.
  * *Sunlight Sensitivity:* Disadvantage on attack rolls and Wisdom (Perception) checks relying on sight in direct sunlight.
* **Actions:**
  * *Dagger:* Melee Weapon Attack: +4 to hit, reach 5 ft., one target. Hit: 4 (1d4 + 2) piercing damage.
  * *Sling:* Ranged Weapon Attack: +4 to hit, range 30/120 ft., one target. Hit: 4 (1d4 + 2) bludgeoning damage.
* **Combat Behavior:** Cowardly. Attacks only with numeric advantage; scatters if an officer falls.

---

## Area 4: Common Anvil Basement (Module Lines 117–126)
* **Encounter Type:** Boss skirmish / Hostage recovery.
* **Evasion / Parley Viability:** Moderate. Skritch will negotiate for his life or retreat if reduced below 20% HP.
* **Ruling 42a Tone Guard:** Combat in a confined cellar with civilian hostages. Collateral damage to the building ends safehouse access.

### Kobold Captain Skritch
*Small humanoid (kobold), lawful evil*
* **Module Source:** `module-0.5.md` L31 ("kobold guard captain Skritch"), L175 (`Kobold Captain Skritch`).
* **Armor Class:** 13 (leather armor, buckler)
* **Hit Points:** 22 (5d6 + 5)
* **Speed:** 30 ft.
* **STR:** 10 (+0) | **DEX:** 15 (+2) | **CON:** 12 (+1) | **INT:** 11 (+0) | **WIS:** 10 (+0) | **CHA:** 12 (+1)
* **Senses:** Darkvision 60 ft., Passive Perception 10
* **Traits:**
  * *Pack Tactics:* Advantage on attack rolls against a creature if an ally is within 5 ft. of the target.
  * *Sunlight Sensitivity:* Disadvantage on attack rolls in direct sunlight.
* **Actions:**
  * *Shortsword:* Melee Weapon Attack: +5 to hit, reach 5 ft., one target. Hit: 5 (1d6 + 2) piercing damage.
  * *Pack Lord's Snarl (Recharge 5–6):* One creature within 30 ft. that can hear Skritch must succeed on a DC 12 Wisdom saving throw or become frightened until the end of Skritch's next turn.
* **Combat Behavior:** Directs minions to flank; targets the heaviest armored foe first; flees at 5 HP or fewer.

### Kobold Minion (3 Present)
*Small humanoid (kobold), lawful evil*
* **Module Source:** `module-0.5.md` L125 ("The three kobold minions rout immediately if Skritch flees"), L189 ("3 kobolds").
* **Armor Class:** 12 | **Hit Points:** 5 | **Speed:** 30 ft.
* **Actions:** Dagger (+4 to hit, 1d4 + 2 piercing) or Sling (+4 to hit, 1d4 + 2 bludgeoning).
* **Combat Behavior:** Holds doorway choke point; routs immediately if Skritch flees.

---

## Assumption Log

### Entry 1 — Assumed Party Level (Ratification-Pending)
* **Claim:** Module-0.5 is balanced for a party of 4 characters at Level 1.
* **Status:** Ratification-pending owner ruling on full campaign level curve.

### Entry 2 — Stat Block Numbers (Ratification-Pending)
* **Claim:** Skritch HP (22), AC (13), and Pack Lord's Snarl DC (12) are derived from 5e CR 1/2 baselines.
* **Status:** Ratification-pending owner ruling.
